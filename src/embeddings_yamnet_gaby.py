# src/embeddings_yamnet_gaby.py
# ------------------------------------------------------------
# YAMNet embeddings pipeline (Xeno-Canto)
# - Notebook-friendly (no heavy execution at import time)
# - Reanudable (skips existing .npz)
# - Logging de errores a CSV
# - Valida que exista la columna `description` en df_final.csv
# ------------------------------------------------------------
...
# Si FFmpeg está instalado en otra ruta, ajusta esto o elimínalo si ya lo tienes en PATH
os.environ["PATH"] += os.pathsep + r"C:\ffmpeg\bin"

CSV_URL = "https://raw.githubusercontent.com/anto-rom/Xeno_Canto_Project/main/data/processed/df_final.csv"

# Recomendación: usar PROJECT_ROOT dinámico si ejecutas desde repo
# (Si prefieres fijo, deja la ruta hardcodeada)
...
# Si quieres cap por especie. None = todo
MAX_PER_SPECIES: Optional[int] = None  # ejemplo: 50
...
def check_ffmpeg() -> None:
    try:
        ...
        raise RuntimeError(
            "❌ ffmpeg no está accesible en PATH. Instálalo o añade su ruta al PATH."
        ) from e
...
def download_audio_streaming(download_url: str, timeout: int = REQUEST_TIMEOUT) -> Path:
    ...
    for attempt in range(1, TRIES_PER_AUDIO + 1):
        try:
            ...
            # sanity mínimo: evita HTML/página de error
...
# ===============================
# AUDIO -> waveform (float32, 16k mono)
# ===============================
...
# ===============================
# YAMNet embeddings + save
# ===============================

def compute_yamnet_embeddings(waveform: np.ndarray, yamnet_model) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    for _, row in tqdm(df_run.iterrows(), total=len(df_run)):
        ...
        try:
            ...
            try:
                ...
                emb, emb_mean, emb_std = compute_yamnet_embeddings(waveform, yamnet_model)
...
def main() -> None:
    ...
    print("Total embeddings files:", stats["n_files"])
    print(f"✅ processed={stats['processed']} | ⏭️ skipped={stats['skipped']} | ❌ failed={stats['failed']}")
    print("Error log:", stats["log_csv"])

    # Mostrar últimos errores si hay
    if Path(stats["log_csv"]).exists():
        err_df = pd.read_csv(stats["log_csv"])
        if len(err_df) > 0:
            print("Errors logged:", len(err_df))
            try:
                display(err_df.tail(10))  # Notebook friendly
            except Exception:
                print(err_df.tail(10))
